...
// WISH LIST PRESIDENTS CONTROLLERS

const allWishPresidents = (req, res, next) => {
  async function run() {
    try {
      await client.connect();
      const database = client.db("Presidents");
      const presidents = database.collection("Wish List Candidates");
      const presidentsJson = await presidents.find({}).toArray((err, lists) => {
        if (err) {
          res.status(400).json({ message: err });
        }
...
const oneWishPresident = (req, res, next) => {
  if (!ObjectId.isValid(req.params.userId)) {
    res
      .status(400)
      .json(
        "ID is not valid. ID must be valid to get one Wish President. Please try again."
      );
  }
  async function run() {
    try {
      await client.connect();
      const database = client.db("Presidents");
      const presidents = database.collection("Wish List Candidate");
      const singlePresidentJson = await presidents.findOne(
        {
          _id: new ObjectId(req.params.userId),
        },
...
const createWishPresident = (req, res, next) => {
  const wishPresident = {
    firstName: req.body.firstName,
    lastName: req.body.lastName,
    party: req.body.party,
...
const updateWishPresident = (req, res, next) => {
  const wishPresident = {
    firstName: req.body.firstName,
    lastName: req.body.lastName,
    party: req.body.party,
...
const deleteWishPresident = (req, res, next) => {
  console.log(req.params.id);
  if (!ObjectId.isValid(req.params.id)) {
    res
      .status(400)
      .json(
        "ID is not valid. ID must be valid to delete one Wish President. Please try again."
      );
  }
  async function run() {
    try {
      ...
      if (result.deletedCount > 0) {
        ...
        res.status(500).json(response.error) ||
          "Some error occured while deleting the Wish President file.";
      }
    } finally {
      await client.close();
    }
